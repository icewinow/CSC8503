...
bool CollisionDetection::CapsuleIntersection(const CapsuleVolume& volumeA, const Transform& worldTransformA, const CapsuleVolume& volumeB,
	...
	ClosestPt(bottomSpherePosA, topSpherePosA, bottomSpherePosB, topSpherePosB, closestPointA, closestPointB);
	float distance = Vector3::Dot(closestPointA - closestPointB, closestPointA - closestPointB);
...
void  CollisionDetection::ClosestPt(const Vector3& p1, const Vector3& q1, const Vector3& p2, const Vector3& q2, Vector3& c1, Vector3& c2) {
	float s, t;
	...
	float SquaredD1 = Vector3::Dot(d1, d1); 
	float SquaredD2 = Vector3::Dot(d2, d2);
	float SquaredR = Vector3::Dot(d2, r);

	if (SquaredD1 <= FLT_EPSILON && SquaredD2 <= FLT_EPSILON) {

		s = t = 0.0f;
		c1 = p1;
		c2 = p2;
	}
	if (SquaredD1 <= FLT_EPSILON) {
		s = 0.0f;
		t = SquaredR / SquaredD2;
		t = Clamp(t, 0.0f, 1.0f);
	}
	else {
		float c = Vector3::Dot(d1, r); if (SquaredD2 <= FLT_EPSILON) {
			t = 0.0f;
			s = Clamp(-c / SquaredD1, 0.0f, 1.0f); 
		}
		else {
			float d1d2 = Vector3::Dot(d1, d2);

			float denom = SquaredD1 * SquaredD2 - d1d2 * d1d2;

			if (denom != 0.0f) {
				s = Clamp((d1d2 * SquaredR - c * SquaredD2) / denom, 0.0f, 1.0f);
			}
			else{
				s = 0.0f;
			}
			float tnom = d1d2 * s + SquaredR;

			if (tnom < 0.0f) {
				t = 0.0f;
				s = Clamp(-c / SquaredD1, 0.0f, 1.0f);
			}
			else if (tnom > SquaredD2) {
				t = 1.0f;
				s = Clamp((d1d2 - c) / SquaredD1, 0.0f, 1.0f);
			}
			else {
				t = tnom / SquaredD2;
			}
