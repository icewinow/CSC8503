...
void PhysicsSystem::ImpulseResolveCollision(GameObject& a, GameObject& b, CollisionDetection::ContactPoint& p) const {
	...
	Vector3 anVelocityA = Vector3::Cross(physA->GetAngularVelocity(), relativeA);
	Vector3 anVelocityB = Vector3::Cross(physB->GetAngularVelocity(), relativeB);

	Vector3 fullVelocityA = physA->GetLinearVelocity() + anVelocityA;
	Vector3 fullVelocityB = physB->GetLinearVelocity() + anVelocityB;
	...
	float aRestitution = physA->GetElasticity();
	float bRestitution = physB->GetElasticity();

	float ja = (-(1.0f + aRestitution) * impulseForce) / (totalMass + angularEffect);
	float jb = (-(1.0f + bRestitution) * impulseForce) / (totalMass + angularEffect);

	Vector3 aImpulse = p.normal * ja;
	Vector3 bImpulse = p.normal * jb;
